<template>
  <div class="rounded-[50px] bg-white w-full p-6">
    <div v-if="pannelToShow == 'none'" class="w-full h-full flex justify-center items-center">
      <p class="text-3xl">choose a task to start the timer !</p>
    </div>
    <div v-if="pannelToShow == 'error'" class="w-full h-full flex justify-center items-center">
      <p class="text-3xl">This task has already been completed !</p>
    </div>
    <div v-if="pannelToShow == 'show'" class="w-full h-full relative overflow-hidden flex justify-center items-center rounded-[40px] bg-[#ffffff]">
...
export default {
  ...
  watch : {
        ...
        timerShown:function(nnn){
          if(nnn){
            const lista = JSON.parse(localStorage.getItem('taskList'))
          let theKnownItem = ''
          lista.forEach((item)=>{
            const itemLoop = item.find((thing)=> thing.id == nnn)
            if(itemLoop){
              theKnownItem = itemLoop
            }
          })
          if(theKnownItem.checked == false){
            this.pannelToShow = 'show'
            let timeRemaining = this.times[this.currentTime].time
        this.minutesDisplay = Math.floor(timeRemaining / 60)
        this.secondsDisplay = timeRemaining % 60
          const lolo = document.querySelector('.blob-cont');
          ...
          if(lolo && bobo){
            ...
            this.pannelToShow = 'error'
            this.stopCounter()
          }
          }else {
            this.pannelToShow = 'none'
            this.stopCounter()
          }
...
.blob-conts {
  ...
  background: linear-gradient(90deg, rgba(255,217,217,1) 0%, rgba(255,244,211,1) 100%);
  transition: all 0.5s;
}
